		...
		?>
				<tr class="<?php echo esc_attr(apply_filters('woocommerce_order_item_class', 'order_item', $item, $subscription)); ?>">
					...
					<td class="product-total">
						<?php echo preg_replace('/ every \d+ years/', '', wp_kses_post($subscription->get_formatted_line_subtotal($item))); ?>
					</td>
				</tr>
			<?php
			}
	...
	<tfoot>
		...
		foreach ($totals as $key => $total) : ?>
			<tr>
				...
				<td><?php echo preg_replace('/ every \d+ years/', '', wp_kses_post($total['value'])); ?></td>
			</tr>
		<?php endforeach; ?>
	</tfoot>
</table>