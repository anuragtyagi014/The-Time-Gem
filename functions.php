    ...
    if ($author > 0 && has_active_subscription() && $author == $author_id) {
        ...
        if (count($error) > 0) {
            ...
            if (isset($data['background-image'])) {
                ...
                if (!empty($_FILES["own_bg_img"])) {
                    $image     = $_FILES["own_bg_img"]; // UploadFile
                    $attachment_id = upload_file_in_wp_media($image);
                    $fileURL      = wp_get_attachment_url($attachment_id);
                    update_post_meta($ID, 'time_gem_bg_image', $fileURL);
                }
            }
...
function send_mail_to_supplier($slug, $image_url, $hollow, $cust_details, $order_id)
{
        ...
        if (($order->has_status('processing') || $order->has_status('completed'))) {
            if ($create_time_gem) {
                ...
                if ($time_gem_exist > 0) {
                    ...
                    send_mail_to_supplier($time_gem_slug, $photo_url[0], $hollow, $cust_details, $order_id);
                    //#comment out 09-11-2023
                ...
                $cust_details['zip'] = $order->get_billing_postcode();
                //comment out 09-11-2023
                send_mail_to_supplier($timegem_post_slug, $plaque_photo_url, $hollow, $cust_details, $order_id);
                //#comment out 09-11-2023
...
// add_action('woocommerce_update_order', 'custom_function_to_do_something_with_tracking_update', 10, 1);

// function custom_function_to_do_something_with_tracking_update($order_id)
// {
...
//     //     'email_heading'   => __('Tracking has applied', 'woocommerce')
//     // ),);
...
//     // ));
//     // // Send the email
//     // wp_mail($order->get_billing_email(), __('Tracking has applied', 'woocommerce'), $email_template, $headers);
//     TestingApply(1);
// }
...
// function TestingApply($id)
// {
//     $to = 'dev.team2080@gmail.com';
//     $subject = 'Hello, this is a simple email';
//     $message = 'This is the content of the email.' . $id;
//     $headers = array('Content-Type: text/html; charset=UTF-8');
...
// // Add action to execute custom function
// add_action('my_custom_secondly_event', 'my_custom_secondly_function');

// // Schedule the event every second
// if (!wp_next_scheduled('my_custom_secondly_event')) {
//     // Add a custom cron schedule for every second
//     add_filter('cron_schedules', 'add_secondly_schedule');
//     function add_secondly_schedule($schedules)
//     {
...
//     // Schedule the event using the custom schedule
//     wp_schedule_event(time(), 'every_second', 'my_custom_secondly_event');
// }

// // Define the function to be executed
// function my_custom_secondly_function()
// {
...
// Add action to execute custom function
add_action('my_custom_minutely_event', 'my_custom_minutely_function');

// Schedule the event every minute
if (!wp_next_scheduled('my_custom_minutely_event')) {
    ...
    // Schedule the event using the custom schedule
    wp_schedule_event(time(), 'every_minute', 'my_custom_minutely_event');
}

// Define the function to be executed
function my_custom_minutely_function()
{
    _sendCustomEmailToCustomerAfterApplyingTrackingSystem();
}

function _sendCustomEmailToCustomerAfterApplyingTrackingSystem()
{
    global $wpdb;
    ...
    $query = $wpdb->prepare("
        ...
        LEFT JOIN {$wpdb->prefix}postmeta AS pm1 ON p.ID = pm1.post_id AND pm1.meta_key = 'is_customer_mail_sent'
        LEFT JOIN {$wpdb->prefix}postmeta AS pm2 ON p.ID = pm2.post_id AND pm2.meta_key = '_wc_shipment_tracking_items'
...
function sendCustomerMailByORDID($order_id)
{
    $order = wc_get_order($order_id);
    // Set up email headers
    $headers = array('Content-Type: text/html; charset=UTF-8');
    ...
    $email_template = wc_get_template_html('emails/email-header.php', array(
        'email_heading'   => __('Tracking Information Applied', 'woocommerce')
    ),);
    $email_template .= wc_get_template_html('emails/email-order-details.php', array(
        'order'           => $order
    ));
    ...
    // Send the email
    if (wp_mail($order->get_billing_email(), __('Tracking Information Applied', 'woocommerce'), $email_template, $headers)) {
        update_post_meta($order_id, 'is_customer_mail_sent', "YES");
    }
}
...
?>
    <script>
        setTimeout(function() {
            let currency = jQuery(".wcc-crnt-currency .wcc-symbol").text().replace(/[{()}]/g, '') ? jQuery(".wcc-crnt-currency .wcc-symbol").text().replace(/[{()}]/g, '') : '$';
            jQuery(".pricingRowWrap .woocommerce-Price-currencySymbol").text(currency);
        }, 1000);

        let timer = setInterval(() => {
            jQuery(".subscription-details").each((idx, item) => {
                if (jQuery(item).text().trim() == " every 100 years".trim()) {
                    jQuery(item).text("");
                    clearInterval(timer);
                }
            });
            if (jQuery(".woocommerce-cart [data-title='Subtotal']").text().includes("every 100 years")) {
                jQuery(".woocommerce-cart [data-title='Subtotal']").html(jQuery("[data-title='Subtotal']").html().replace("every 100 years", ""));
            }

            jQuery(".woocommerce-cart .subscription-details").css("opacity", 1);
            jQuery(".woocommerce-cart [data-title='Subtotal']").css("opacity", 1);
        }, 100);
    </script>
<?php
});
...
add_action('mc4wp_form_subscribed', 'custom_mc4wp_subscribe_email');

function custom_mc4wp_subscribe_email($subscriber_id)
{

    // $subscriber = mc4wp_get_subscriber($subscriber_id);

    // Ensure subscriber exists and email is present
    ...
    $message = 'Dear ' . $name . ',<br><br>Thank you for subscribing to our newsletter!<br><br>Best regards,<br>The Time Gem';

    // Set up headers to ensure HTML email
